...
class Meting {
  ...
  async _netease_AESCBC(api) {
    ...
    // RSA using BigInt
    let skeyReversed = Buffer.from(skey, 'utf-8').toString('binary');
    skeyReversed = skeyReversed.split('').reverse().join('');
    const skeyHex = this._str2hex(skeyReversed);
    let skeyInt = this._bchexdec(skeyHex);
    skeyInt = (skeyInt ** BigInt(pubkey)) % this._bchexdec(modulus);
    let skeyFinal = this._bcdechex(skeyInt);
    while (skeyFinal.length < 256) {
      skeyFinal = '0' + skeyFinal;
    }

    api.url = api.url.replace('/api/', '/weapi/');
    api.body = { params: body, encSecKey: skeyFinal };
    return api;
  }
