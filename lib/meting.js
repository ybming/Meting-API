...
class Meting {
  ...
  async _tencent_url(result) {
    ...
    const respData = response.req_0 && response.req_0.data ? response.req_0.data : {};
    const sip = respData.sip || [];
    const vkeys = respData.midurlinfo || [];
    let urlObj = { url: '', size: 0, br: -1 };

    // Primary: try vkey + purl for each quality level
    for (let i = 0; i < typeList.length; i++) {
      const vo = typeList[i];
      if (data.data[0].file && data.data[0].file[vo[0]] && vo[1] <= (this.temp.br || 320)) {
        if (vkeys[i] && vkeys[i].purl && sip[0]) {
          urlObj = {
            url: sip[0] + vkeys[i].purl,
    ...
    // Fallback: use testfile2g/testfilewifi when vkey is blocked (e.g. overseas IP)
    if (!urlObj.url && sip[0]) {
      const testFile = respData.testfile2g || respData.testfilewifi;
      if (testFile) {
        urlObj = {
          url: sip[0] + testFile,
          size: data.data[0].file ? (data.data[0].file.size_96aac || data.data[0].file.size_128mp3 || 0) : 0,
          br: 96,
        };
      }
    }
